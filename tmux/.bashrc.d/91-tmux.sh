...
if [ -n "$PS1" ] && [ -z "$TMUX" ]; then
    ...
    if [ "$TERM_PROGRAM" = "vscode" ]; then
        ...
        # echo "Starting or attaching to tmux session '$SESSEION_NAME'"
        # tmux new-session -A -s "$SESSEION_NAME"
        echo "You are inside VSCode."
        echo "Tmux will not be started automatically. You can start it with 'tmux new-session -A -s <session-name>'"
    else
        ...
        tmux new-session -A -s Root-Session
    fi
